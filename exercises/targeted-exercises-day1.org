...
course_packages  <- c("targets","data.table","ranger","glmnet","Publish","lava","riskRegression","matrixStats","ggplot2")
...
- Add the following line in the file =functions/this.R= inside the
  definition =(1=2)=. Then run =tar_make()= and see what happens and
  follow the suggestion of the error message:
  
 Tips: https://books.ropensci.org/targets/debugging.html

 here in particular Sections 3.4 Error messages and 3.5 Functions. 
...
Introduce a targets pipeline into one of your own projects.
