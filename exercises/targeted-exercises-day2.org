...
The folder [[example-project]] contains a data set and the R-code file
[[example-project/rate-versus-risks.R]] which loads the data, prepares the
data and then calculates rates, rate ratios and t-year risks and
t-year risk ratios.

Translate the R-codes of the file [[example-project/rate-versus-risks.R]]  into a new targets project as follows:
...
5. Copy the data that are read by [[example-project/rate-versus-risks.R]] to the new subfolder =data=
6. Copy all functions from the file  [[example-project/rate-versus-risks.R]] to the new subfolder =functions=
7. Create the targets pipeline in the file =_targets.R= by:
   ...
   copying/translating all the remaining R-codes into targets.
   Don't do all of them at once. Do them one-by-one and check in between if 
    =tar_make()= runs without errors.
